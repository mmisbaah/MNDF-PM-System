...
$project=[System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot ".."));. (Join-Path $PSScriptRoot 'resolve-node-runtime.ps1');$node=Resolve-PerformanceTrackerNode -ProjectDirectory $project
...
$results=[System.IO.Path]::GetFullPath((Join-Path $project $ResultDirectory));New-Item -ItemType Directory -Force -Path $results|Out-Null;$run=Join-Path $results "restore-$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ'))";New-Item -ItemType Directory -Path $run|Out-Null
