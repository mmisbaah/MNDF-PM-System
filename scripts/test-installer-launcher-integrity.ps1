...
$failed=$false;try{& $launcher @common -ApprovedLauncherSha256 ('0'*64) -ApprovedVerifierSha256 $verifierHash -ApprovedAclHelperSha256 $helperHash}catch{$failed=$_.Exception.Message-match'launcher fingerprint'}
if(-not$failed){throw 'Installer launcher accepted an unapproved self fingerprint'}
$failed=$false;try{& $launcher @common -ApprovedLauncherSha256 $launcherHash -ApprovedVerifierSha256 ('0'*64) -ApprovedAclHelperSha256 $helperHash}catch{$failed=$_.Exception.Message-match'verifier fingerprint'}
