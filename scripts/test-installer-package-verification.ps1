...
try{
  ...
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40) -ApprovedCompilerSha256 ('b'*64)}catch{$failed=$_.Exception.Message-match'approved signtool\.exe'}
