...
try{
  ...
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath}catch{$failed=$_.Exception.Message-match'approved code-signing certificate'}
  if(-not$failed){throw 'Production verification accepted a missing approved signer thumbprint'}
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40)}catch{$failed=$_.Exception.Message-match'approved RFC 3161 timestamp certificate'}
  if(-not$failed){throw 'Production verification accepted a missing approved timestamp certificate'}
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40)}catch{$failed=$_.Exception.Message-match'approved application version'}
  if(-not$failed){throw 'Production verification accepted a missing approved application version'}
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40) -ApprovedAppVersion '9.9.9'}catch{$failed=$_.Exception.Message-match'Installer version does not match'}
  if(-not$failed){throw 'Production verification accepted a different approved application version'}
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40) -ApprovedAppVersion '0.1.0-test'}catch{$failed=$_.Exception.Message-match'approved release commit'}
  ...
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40) -ApprovedAppVersion '0.1.0-test' -ApprovedReleaseCommit ('a'*40) -ApprovedCompilerSha256 ('b'*64)}catch{$failed=$_.Exception.Message-match'approved signtool\.exe'}
  ...
  $failed=$false;try{& (Join-Path $PSScriptRoot 'verify-installer-package.ps1') -InstallerPath $installer -BuildRecordPath $recordPath -ApprovedSigningCertificateThumbprint ('f'*40) -ApprovedTimestampCertificateThumbprint ('2'*40) -ApprovedAppVersion '0.1.0-test' -ApprovedReleaseCommit ('a'*40) -ApprovedCompilerSha256 ('b'*64) -ApprovedSignToolSha256 ('1'*64)}catch{$failed=$_.Exception.Message-match'approved release public-key'}
