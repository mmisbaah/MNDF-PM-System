...
foreach($entry in $paths.GetEnumerator()){
  ...
  foreach($rule in $acl.Access){
    ...
    if($identity-eq$ServiceAccount-and$rule.AccessControlType-eq'Allow'){$serviceRights=$serviceRights-bor$rule.FileSystemRights}
...
$result=[ordered]@{format="performance-tracker-acl-verification-v1";status=if($failures.Count){'FAIL'}else{'PASS'};checkedAt=(Get-Date).ToUniversalTime().ToString('o');serviceAccount=$ServiceAccount;paths=$paths;failures=$failures}
