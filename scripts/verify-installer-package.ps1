...
if($record.format-ne'performance-tracker-installer-build-v3'){throw 'Version 3 installer provenance is required'}
if([IO.Path]::GetFullPath([string]$record.path)-ne$installer-or[IO.Path]::GetFullPath([string]$record.recordPath)-ne$recordPath){throw 'Installer provenance paths do not match the supplied artifacts'}
...
if($AllowUnsignedRehearsal){
  if($record.productionAuthorized-ne$false-or$record.authenticodeStatus-ne'NotSigned'-or$null-ne$record.recordSignaturePath){throw 'Artifact is not a valid unsigned rehearsal bundle'}
} else {
  ...
  $recordSignature=[IO.Path]::GetFullPath([string]$record.recordSignaturePath)
