...
Before transferring or running an installer, use `scripts/verify-installer-package.ps1` with explicit installer, build-record, public-key, approved Node-runtime path, and the independently recorded `-ApprovedInstallerSha256`, `-ApprovedAppVersion`, `-ApprovedReleaseCommit`, `-ApprovedSigningCertificateThumbprint`, `-ApprovedTimestampCertificateThumbprint`, `-ApprovedSignToolSha256`, and `-ApprovedReleasePublicKeySha256`. The reviewed Inno Setup and Node.js fingerprints are pinned by `-ApprovedCompilerSha256` and `-ApprovedNodeRuntimeSha256`. Production verification checks the exact authorized installer digest, version and source commit, EXE Authenticode signature, actual signer and timestamp authority, release tools, runtime hash and OpenJS publisher, and Ed25519 trust key against those independent approvals. It validates every recorded fingerprint, checks the exact adjacent sidecar signature path, and only then executes Node to verify the record signature. These independent values prevent a valid historical installer, rebuilt substitute, or its signed record from authorizing its own promotion. `-AllowUnsignedRehearsal` accepts only records explicitly marked non-production and never converts them into deployable artifacts.
