...
Before transferring or running an installer, place the three production artifacts in a dedicated handoff directory with ACL inheritance disabled and write access limited to SYSTEM, local Administrators, and explicitly approved custodians. Then use `scripts/verify-installer-package.ps1` with `-ApprovedPackageCustodians`, explicit installer, build-record, public-key, approved Node-runtime path, and the independently recorded `-ApprovedInstallerSha256`, `-ApprovedAppVersion`, `-ApprovedReleaseCommit`, `-ApprovedReleaseId`, `-ApprovedSigningCertificateThumbprint`, `-ApprovedTimestampCertificateThumbprint`, `-ApprovedSignToolSha256`, and `-ApprovedReleasePublicKeySha256`. Production verification rejects inherited handoff permissions or write-capable ACL entries for any unapproved identity. It snapshots all three artifact hashes and confirms they remain unchanged after cryptographic verification. It checks the exact authorized installer digest, version, source commit and installed release identifier, EXE Authenticode signature, actual signer and timestamp authority, release tools, runtime hash and OpenJS publisher, and Ed25519 trust key against those independent approvals. These controls prevent a valid historical installer, rebuilt substitute, renamed deployment identity, mutable handoff bundle, or its signed record from authorizing its own promotion. `-AllowUnsignedRehearsal` accepts only records explicitly marked non-production and never converts them into deployable artifacts.
