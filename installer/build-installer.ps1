...
$sbomPath=Join-Path $release '.next\standalone\sbom.cdx.json'
if(-not(Test-Path -LiteralPath $sbomPath -PathType Leaf)){throw 'Prepared release is missing the runtime SBOM'}
$sbom=Get-Content -LiteralPath $sbomPath -Raw|ConvertFrom-Json
if($sbom.bomFormat-ne'CycloneDX'-or$sbom.specVersion-ne'1.6'-or$sbom.metadata.component.name-ne'performance-tracker'){throw 'Prepared release runtime SBOM is invalid'}
...
$nodeRuntimeSignature=Get-AuthenticodeSignature -LiteralPath $nodeExecutable
if($nodeRuntimeSignature.Status-ne'Valid'-or$nodeRuntimeSignature.SignerCertificate.Subject-notmatch'(^|,\s*)O=OpenJS Foundation(,|$)'){throw 'Portable Node.js runtime must have a valid OpenJS Foundation Authenticode signature'}
$nodeRuntimeSigner=$nodeRuntimeSignature.SignerCertificate.Subject
...
try {
  if($AllowUnsignedRehearsal){
    ...
    if(-not$TrustedTimestampCertificateThumbprint){throw 'The independently approved RFC 3161 timestamp certificate thumbprint is required'}
