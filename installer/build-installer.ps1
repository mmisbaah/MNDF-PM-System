...
& node (Join-Path $release 'scripts\release-signing.mjs') verify (Join-Path $release '.next\standalone\release-manifest.json') (Join-Path $release '.next\standalone\release-manifest.sig.json') $publicKey
...
$defines=@("/DReleaseSource=$release","/DReleaseId=$ReleaseId","/DAppVersion=$AppVersion","/DReleasePublicKey=$publicKey","/DTrustedPublicKeySha256=$publicKeySha256","/O$output")
...
[ordered]@{format='performance-tracker-installer-build-v1';path=$installer.FullName;sha256=(Get-FileHash -LiteralPath $installer.FullName -Algorithm SHA256).Hash.ToLowerInvariant();releaseCommit=$manifest.commit;releaseId=$ReleaseId;version=$AppVersion;releasePublicKeySha256=$publicKeySha256;authenticodeStatus=[string]$signature.Status;productionAuthorized=(-not$AllowUnsignedRehearsal)}|ConvertTo-Json -Depth 3
