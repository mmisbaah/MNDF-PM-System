...
ALTER TABLE disciplinary_matters ADD COLUMN data_mode data_mode NOT NULL DEFAULT 'PRODUCTION';
ALTER TABLE awol_incidents ADD COLUMN data_mode data_mode NOT NULL DEFAULT 'PRODUCTION';

CREATE TRIGGER disciplinary_matters_apply_data_mode BEFORE INSERT ON disciplinary_matters
FOR EACH ROW EXECUTE FUNCTION apply_active_data_mode();
CREATE TRIGGER awol_incidents_apply_data_mode BEFORE INSERT ON awol_incidents
FOR EACH ROW EXECUTE FUNCTION apply_active_data_mode();

CREATE INDEX idx_disciplinary_reporting_mode ON disciplinary_matters (tenant_id,data_mode,personnel_id,status);
CREATE INDEX idx_awol_reporting_mode ON awol_incidents (tenant_id,data_mode,personnel_id,status);

-- Training incidents must never affect production promotion or commendation eligibility.
...
RETURNS void LANGUAGE plpgsql AS $$ DECLARE v_app record; v_score numeric; v_bad boolean; v_open boolean; v_eligible boolean; v_type recommendation_type; BEGIN
  SELECT a.id,a.score_percentage,p.unit_service_started_on INTO v_app FROM appraisals a
    JOIN evaluation_cycles cy ON cy.tenant_id=a.tenant_id AND cy.id=a.cycle_id AND cy.cycle_type='ANNUAL' AND cy.data_mode='PRODUCTION'
  ...
  SELECT EXISTS(SELECT 1 FROM disciplinary_matters WHERE tenant_id=p_tenant AND personnel_id=p_personnel AND data_mode='PRODUCTION' AND status IN ('ALLEGED','UNDER_REVIEW','CONFIRMED'))
    OR EXISTS(SELECT 1 FROM awol_incidents WHERE tenant_id=p_tenant AND personnel_id=p_personnel AND data_mode='PRODUCTION' AND status IN ('REPORTED','UNDER_REVIEW','CONFIRMED')) INTO v_open;
